    ...
    async def generate_post_for_user(self, user_id: str, topic_id: str) -> Optional[GeneratedPost]:
        ...
        try:
            ...
            if response.msgs and len(response.msgs) > 0:
                ...
                # 正则化去除<think>内容，只保留生成的帖子正文
                import re
                # 只保留最后一个</think>之后的内容
                if "<think>" in content and "</think>" in content:
                    # 匹配最后一个</think>后的内容
                    parts = re.split(r"</think>\s*", content)
                    if len(parts) > 1:
...
async def main():
    ...
    # 加载数据
    users = generator.load_users_data("data/raw/users_info.csv")
    topics = generator.load_topics_data("data/processed/topics.json")
    ...
    # 保存结果
    generator.save_posts_to_json(posts, "data/processed/generated_posts.json")
    generator.save_posts_to_csv(posts, "data/processed/generated_posts.csv")
